"use client"
import { motion } from "framer-motion"
import { useStore } from "@/app/z-module/store"
const { visibleFormConsultation } = useStore

export default function ProductList() {
  const { openVisibleFormConsultation } = visibleFormConsultation();

  return (
        <motion.main 
          initial={{ opacity: 0 }} 
          animate={{ opacity: 2, y: -10 }} 
          transition={{ type: "spring", stiffness: 50 }}
        >
          <section class="bg-white dark:bg-gray-900 antialiased">
                <div class="max-w-screen-xl px-4 py-4 mx-auto sm:py-6 ">
                  <div class="grid grid-cols-1 gap-4 mt-6 lg:mt-8 sm:grid-cols-2 md:grid-cols-3">
                    <div class="relative overflow-hidden rounded-lg group">
                      <img class="object-cover w-full h-[240px]  scale-100 ease-in duration-300 group-hover:scale-125" src="https://zovrus.ru/files/_cache/product/chr-0670_2.resize1200x1200w.jpg?5c92da30a4854ee4a81e8a9dcb3c75ed" alt=""/>
                      <div class="absolute inset-0 grid items-end justify-center p-4 bg-gradient-to-b from-transparent to-black/60">
                        <div class="text-center">
                          {/* <span class="text-xl font-bold text-white">
                            Проект: РШ 8327
                          </span> */}
                          <p class="text-base font-medium text-gray-300">
                            Проект: РШ 8327
                          </p>
                        </div>
                      </div>
                    </div>
                    <div class="relative overflow-hidden rounded-lg group">
                      <img class="object-cover w-full h-[240px]  scale-100 ease-in duration-300 group-hover:scale-125" src="https://zovrus.ru/files/_cache/product/chr-0670_2.resize1200x1200w.jpg?5c92da30a4854ee4a81e8a9dcb3c75ed" alt=""/>
                      <div class="absolute inset-0 grid items-end justify-center p-4 bg-gradient-to-b from-transparent to-black/60">
                        <div class="text-center">
                          {/* <span class="text-xl font-bold text-white">
                            Проект: РШ 8327
                          </span> */}
                          <p class="text-base font-medium text-gray-300">
                            Проект: РШ 8327
                          </p>
                        </div>
                      </div>
                    </div>
                    <div class="relative overflow-hidden rounded-lg group">
                      <img class="object-cover w-full h-[240px]  scale-100 ease-in duration-300 group-hover:scale-125" src="https://zovrus.ru/files/_cache/product/chr-0670_2.resize1200x1200w.jpg?5c92da30a4854ee4a81e8a9dcb3c75ed" alt=""/>
                      <div class="absolute inset-0 grid items-end justify-center p-4 bg-gradient-to-b from-transparent to-black/60">
                        <div class="text-center">
                          {/* <span class="text-xl font-bold text-white">
                            Проект: РШ 8327
                          </span> */}
                          <p class="text-base font-medium text-gray-300">
                            Проект: РШ 8327
                          </p>
                        </div>
                      </div>
                    </div>
                    <div class="relative overflow-hidden rounded-lg group">
                      <img class="object-cover w-full h-[240px]  scale-100 ease-in duration-300 group-hover:scale-125" src="https://zovrus.ru/files/_cache/product/chr-0670_2.resize1200x1200w.jpg?5c92da30a4854ee4a81e8a9dcb3c75ed" alt=""/>
                      <div class="absolute inset-0 grid items-end justify-center p-4 bg-gradient-to-b from-transparent to-black/60">
                        <div class="text-center">
                          {/* <span class="text-xl font-bold text-white">
                            Проект: РШ 8327
                          </span> */}
                          <p class="text-base font-medium text-gray-300">
                            Проект: РШ 8327
                          </p>
                        </div>
                      </div>
                    </div>
                    <div class="relative overflow-hidden rounded-lg group">
                      <img class="object-cover w-full h-[240px]  scale-100 ease-in duration-300 group-hover:scale-125" src="https://zovrus.ru/files/_cache/product/chr-0670_2.resize1200x1200w.jpg?5c92da30a4854ee4a81e8a9dcb3c75ed" alt=""/>
                      <div class="absolute inset-0 grid items-end justify-center p-4 bg-gradient-to-b from-transparent to-black/60">
                        <div class="text-center">
                          {/* <span class="text-xl font-bold text-white">
                            Проект: РШ 8327
                          </span> */}
                          <p class="text-base font-medium text-gray-300">
                            Проект: РШ 8327
                          </p>
                        </div>
                      </div>
                    </div>
                    <div class="relative overflow-hidden rounded-lg group">
                      <img class="object-cover w-full h-[240px]  scale-100 ease-in duration-300 group-hover:scale-125" src="https://zovrus.ru/files/_cache/product/chr-0670_2.resize1200x1200w.jpg?5c92da30a4854ee4a81e8a9dcb3c75ed" alt=""/>
                      <div class="absolute inset-0 grid items-end justify-center p-4 bg-gradient-to-b from-transparent to-black/60">
                        <div class="text-center">
                          {/* <span class="text-xl font-bold text-white">
                            Проект: РШ 8327
                          </span> */}
                          <p class="text-base font-medium text-gray-300">
                            Проект: РШ 8327
                          </p>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div class="max-w-3xl p-4 mx-auto mt-8 rounded-md lg:mt-16 bg-gray-50 dark:bg-gray-800">
                    <div class="flex flex-col justify-between gap-3 md:gap-6 md:items-center md:flex-row">
                      <div class="flex items-center gap-1.5">
                        <span class="text-base font-normal text-gray-700 dark:text-gray-400">
                          <span class="font-semibold">
                            Вы можете вызвать замерщика на объект.
                          </span>
                        </span>
                      </div>

                      <button onClick={openVisibleFormConsultation}
                        class="inline-flex items-center text-base font-medium text-red-600 hover:underline dark:text-red-500">
                        Записаться на замер
                        <svg aria-hidden="true" class="w-5 h-5 ml-1.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                          fill="currentColor">
                          <path fill-rule="evenodd"
                            d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                            clip-rule="evenodd" />
                        </svg>
                      </button>
                    </div>
                  </div>
                </div>
          </section>
        </motion.main>
  )
}
